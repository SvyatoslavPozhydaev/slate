...
var Editable = props => {
  ...
  , Object.assign({
    ...
    onBeforeInput: React.useCallback(event => {
      ...
      if (IS_FIREFOX && !readOnly && !isEventHandled(event, attributes.onBeforeInput) && hasEditableTarget(editor, event.target)) {
    ...
    onPaste: React.useCallback(event => {
      ...
      if ((IS_FIREFOX || isPlainTextOnlyPaste(event.nativeEvent)) && !readOnly && hasEditableTarget(editor, event.target) && !isEventHandled(event, attributes.onPaste)) {
        event.preventDefault();
